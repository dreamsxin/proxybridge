...
func (c *bridgeCollector) Collect(ch chan<- prometheus.Metric) {
	...
	ch <- prometheus.MustNewConstMetric(c.rejectedTotal, prometheus.CounterValue, float64(metricTotals.RejectedClosing.Load()), "closing")
	...
	for _, snapshot := range snapshots {
		...
		ch <- prometheus.MustNewConstMetric(c.listenerRejected, prometheus.CounterValue, float64(snapshot.RejectedClosing), append(labels, "closing")...)
